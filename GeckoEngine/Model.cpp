...
void Model::loadMesh(unsigned int indexMesh)
{
	...
	unsigned int norAccInd = JSON["meshes"][indexMesh]["primitives"][0]["attributes"]["NORMAL"];
	...
	std::vector<float> norVec = getFloats(JSON["accessors"][norAccInd]);
	std::vector<float> texVec = getFloats(JSON["accessors"][texAccInd]);
	std::vector<glm::vec3> positions = groupFLoatsVec3(posVec);
	std::vector<glm::vec3> normals   = groupFLoatsVec3(norVec);
...
void Model::traverseNode(unsigned int nextNode, glm::mat4 matrix)
{
	...
	if (node.find("matrix") != node.end())
	{
		...
		matNode = glm::make_mat4(matrixValues);
	}
...
std::vector<float> Model::getFloats(json accessor)
{
	...
	for (unsigned int i = beginningOfData; i < beginningOfData + lengthOfData; ) // CHECK this line
	{
...
std::vector<Vertex> Model::assembleVertices(std::vector<glm::vec3> positions, std::vector<glm::vec3> normals, std::vector<glm::vec2> texUVs)
{
	...
	glm::vec3 defaultColorWhite = glm::vec3(1.0f, 1.0f, 1.0f);
	for (int i = 0; i < positions.size(); i++)
	{
		vertices.push_back(Vertex{ positions[i], normals[i], defaultColorWhite, texUVs[i] });
	}
	return vertices;
}
